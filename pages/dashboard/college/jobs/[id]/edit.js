...
const JobAdd = ({ job, user }) => {
  ...
  const [allowPlaced, setAllowPlaced] = useState(job?.allowPlaced);
  ...
  const [typeOfProgram, setTypeOfProgram] = useState(job?.program);
  ...
  useEffect(() => {
    if (selectedDiplomaTypeOfGrade.name === "CGPA")
      getSelectedGrade(CGPAs, job?.eligibility?.diploma?.grade);
  ...
  const isPrevComplete = (roundIndex) => {
    if (roundIndex === 0) return true;

    return (
      roundIndex < rounds.length &&
      (rounds[roundIndex - 1].status === "Partially completed" ||
